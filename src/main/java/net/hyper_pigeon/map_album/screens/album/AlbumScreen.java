...
public class AlbumScreen extends Screen {
    ...
    public final List<Pair<Integer, MapState>> mapsInfo;
    ...
    public AlbumScreen(Text title, List<Pair<Integer, MapState>> mapsInfo) {
        super(title);
    ...
    protected void addCloseButton() {
        this.addDrawableChild(ButtonWidget.builder(ScreenTexts.DONE, button -> this.close()).dimensions(this.width / 2 - 100, 235, 200, 20).build());
    }

    protected void addPageButtons() {
        int i = (this.width - 192) / 2;
        int j = 2;
        this.nextPageButton = this.addDrawableChild(new PageTurnWidget(i + 116, 215, true, button -> this.goToNextPage(), true));
        this.previousPageButton = this.addDrawableChild(
                new PageTurnWidget(i + 43, 215, false, button -> this.goToPreviousPage(), true)
        );
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        drawBackground(ctx,(this.width/2)- 90,(this.height/15));
        if(this.mapsInfo.size() > 0) {
            Pair<Integer, MapState> mapInfo = this.mapsInfo.get(pageIndex);
            drawMap(ctx,mapInfo.getLeft(),mapInfo.getRight(),(this.width/2) - 80,(this.height/15)+10,1.25F);
        }
        else {
            String text = "No maps :(";
            ctx.getMatrices().push();
            ctx.getMatrices().translate((this.width-client.textRenderer.getWidth(text))/2F - 10F, (this.height/15F)+30,0F);
